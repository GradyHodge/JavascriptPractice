...
module.exports = {
  findAll: function (req, res) {
    console.log("Find all campaigns request.");
    db.Campaign.find(req.query)
      .then((dbCampaign) => res.json(dbCampaign))
      .catch((err) => res.status(422).json(err));
  },
  findById: function (req, res) {
    console.log("Find campaign by id request.");
  ...
  update: function (req, res) {
    ...
    if (!req.params.id || req.body === {}) {
      console.log("Missing data in campaign update request.");
      res.send({
      ...
      console.log(
        "Find one campaign and update request.",
        req.params.id,
        req.body
      );
      db.Campaign.findOneAndUpdate({ _id: req.params.id }, req.body)
        .catch((err) =>
  ...
  remove: function (req, res) {
    console.log("Remove campaign request.");
    // If a id is present then run delete
    if (req.params.id) {
      db.Campaign.findById(req.params.id)
        .then((dbCampaign) => dbCampaign.remove())
        .catch((err) =>
          res.send({
